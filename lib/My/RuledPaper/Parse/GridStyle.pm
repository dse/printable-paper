...
sub parseGridStyle {
    ...
    die("invalid grid style: $style (minor $minor is too small)") if $minor < 1;
    die("invalid grid style: $style (minor $minor is too large)") if $minor > 32;
    die("invalid grid style: $style (major $major is too small)") if $major < 1;
    die("invalid grid style: $style (major $major is too large)") if $major > 32;
    die("invalid grid style: $style (major $major is not greater than minor $minor)") if $major <= $minor;
    die("invalid grid style: $style (major $major is not a multiple of minor $minor)") if $major % $minor != 0;
    if (!defined $major) {
        $major = $minor;
        $minor = undef;
    }
